...
class StandardBPE:
    ...
    def __init__(self, vocab_size=8192, pct_bpe=1.0,
                 ngram_min=2, ngram_max=2, required_tokens=None,
                 strict=False,
        ...
        self.word_vocab_size = max(
            [int(vocab_size * (1 - pct_bpe)),
            len(self.required_tokens or [])]
            )
        # set pct_bpe=1.0 to do pure bpe
    ...
    def byte_pair_counts(self, words):
        # type: (StandardBPE, Iterable[str]) -> Iterable[Counter]
    ...
    def count_tokens(self, words):
        # type: (StandardBPE, Iterable[str]) -> Dict[str, int]
    ...
    def learn_bpe_vocab(self, words):
        ...
        vocab = Counter()
        # ensure that SOW and EOW are included
    ...
    def fit(self, text):
        ...
        remaining_words = [
            word for word in toolz.concat(map(self.word_tokenizer, _text))
    ...
    def transform(self, sentences, reverse=False, fixed_length=None):
        # type: (StandardBPE, Iterable[str], bool, int) -> Iterable[List[int]]
    ...
    def inverse_transform(self, rows):
        # type: (StandardBPE, Iterable[List[int]]) -> Iterator[str]
    ...
    def vocabs_to_dict(self, dont_warn=False):
        # type: (StandardBPE, bool) -> Dict[str, Dict[str, int]]
    ...
    def save(self, outpath, dont_warn=False, encoding=None, ensure_ascii=True, indent=2):
        # type: (StandardBPE, str, bool, bool, bool) -> None
    ...
    def from_dict(cls, vocabs):
        # type: (Any, Dict[str, Dict[str, int]]) -> StandardBPE
        """ Load encoder from dict produced with vocabs_to_dict """
        encoder = StandardBPE(**vocabs['kwargs'])
